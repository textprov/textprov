...
class TestCli(unittest.TestCase):
    ...
    def test_mark_convert_strip_round_trip(self):
        ...
        self.assertEqual(
            self.run_cli("strip", self.write("p.txt", pua)), source
        )

    def test_mark_state_and_mode_flags(self):
        path = self.write("s.txt", "Hi")
        self.assertEqual(
            self.run_cli("mark", "--human", path), textprov.mark("Hi", "human")
        )
        self.assertEqual(
    ...
    def test_render_and_inspect(self):
        ...
        self.assertEqual(
            self.run_cli("render", path), textprov.to_html("a" + AI)
        )
        self.assertIn(
            'class="x x-ai"',
            self.run_cli("render", "--class-prefix", "x", path),
        )
